...
int process_cli_requests(int svr_socket) {
    ...
    while (1) {
        cli_socket = accept(svr_socket, NULL, NULL);  // Accept a client connection
        if (cli_socket < 0) {
            perror("accept");
            return ERR_RDSH_COMMUNICATION;
        }
        ...
        if (rc == STOP_SERVER_SC) {
            break;  // Stop server when "stop-server" is received
        }
    }

    stop_server(svr_socket);  // Stop the server after processing all clients
    return rc;
}
...
int exec_client_requests(int cli_socket) {
    ...
    char *io_buff = malloc(RDSH_COMM_BUFF_SZ);  // Buffer for receiving data
    int rc;
    ...
    while (1) {
        ...
        if (io_size <= 0) {
            break;  // Exit if the client disconnects or there is an error
        }

        // Build the command list from received data
        build_cmd_list(io_buff, &cmd_list);

        // Execute the command and return the result
        rc = rsh_execute_pipeline(cli_socket, &cmd_list);
        if (rc == STOP_SERVER_SC) {
            send_message_eof(cli_socket);  // Send EOF when stopping the server
            break;
        }

        // Send EOF after command execution
...
int send_message_string(int cli_socket, char *buff) {
    ...
    // Send the EOF character to indicate end of stream
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    int pids_st[clist->num];  // Array to store process exit statuses
